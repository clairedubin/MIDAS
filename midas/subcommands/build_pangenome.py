...
from midas.common.utils import tsprint, InputStream, OutputStream, retry, command, hashmap, split, multiprocessing_map, multithreading_map, num_vcpu, select_from_tsv, transpose, find_files, upload, upload_star, flatten, pythonpath, num_physical_cores, copy_star
from midas.common.utilities import decode_species_arg, has_ambiguous_bases
...
def build_pangenome_master(args):
    ...
    def species_work(species_id):
        ...
        with CONCURRENT_SPECIES_BUILDS:
            ...
            worker_cmd = f"cd {worker_subdir}; PYTHONPATH={pythonpath()} {sys.executable} -m midas build_pangenome -s {species_id} {subcmd_str} {'--recluster' if args.recluster else ''} &>> {worker_log}"
